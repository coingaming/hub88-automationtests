...
import { getLastWeekRange } from '../utils/timeRangeGenerator.js';
...
export const options = {
  ...
  tags: {
    name: 'Last Week with Transaction Query',
...
// Function to generate a payload with last week time range and transaction UUID
function generatePayload() {
  // Get dynamic time range for last week
  const timeRange = getLastWeekRange();
  
  // Build filter object
  const filter = {
    startTime: timeRange.startTime,
    endTime: timeRange.endTime,
    transactionUuid: environment.transactionUuid
  };
...
export default function() {
  ...
  // Generate the payload with last week time range and transaction UUID
  const payload = generatePayload();
  
  // Log request details for debugging
  // console.log(`Making request to: ${url}`);
  // console.log(`Time range: ${payload.variables.filter.startTime} to ${payload.variables.filter.endTime}`);
  // console.log(`Transaction UUID: ${payload.variables.filter.transactionUuid}`);
