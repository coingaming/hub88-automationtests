...
// Assert that game_code appears at most 50 times in the response
pm.test('game_code appears at most 50 times in the response', () => {
    pm.expect(response.games.filter(game => game.game_code).length).to.be.at.most(50);
});
...
pm.test("Verify the quantity of game_code in the response matches the limit from the request", function () {
    ...
    pm.expect(gameCodesCount).to.equal(50);
});