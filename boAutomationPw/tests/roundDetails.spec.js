...
const endpoint = 'https://bo-v3.hub88.io/transactions'; // <-- Set your endpoint here
...
test.describe('Supplier roundDetails GraphQL check', () => {
  for (const supplier of suppliers) {
    test(`Check roundDetails for supplier: ${supplier}`, async ({ page }) => {
      ...
      // Select supplier by visible text in span
      const supplierSpan = page.locator('xpath=/html/body/div/div/div/main/div/div[1]/div[2]/div/div/div/div[3]/div/div/div[1]/div/div/span', { hasText: supplier });
      await expect(supplierSpan).toBeVisible();
      await supplierSpan.click();
