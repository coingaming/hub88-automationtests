import { baseUrl, operatorID, generateSignature, currencyCode, gameCode, countryCode, langCode, loadPrivateKey } from '../../config/operatorConfigASIAGAMING';
              ...
              cy.then(async () => {
                ...
                .then((response) => {
                  ...
                  cy.then(() => cy.step('🔗 Validating response URL format')).then(() => {
                    ...
                    expect(response.body.url).to.include('launcher-as1.fh8labs.com/games', 'Expected URL to contain launcher-as1.fh8labs.com/games');
                  });
                });
              });
            });
